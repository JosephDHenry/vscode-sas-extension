...
export class ComputeSession extends Compute {
  ...
  async getState(options?: stateOptions): Promise<AxiosResponse<string>> {
    ...
    return this.api.getSessionState(parms);
  }
   ...
   * @param state
   */
  async setState(state: ComputeState): Promise<void> {
    await this.api.updateSessionState({
      sessionId: this.sessionId,
      value: state,
      ifMatch: this.etag,
    });
  ...
  async cancel(): Promise<boolean> {
    if (this._self.links === undefined) await this.self();
    const link = this.getLink(this._self.links, "cancel");
    if (link === undefined)
      throw new Error("Session does not have cancel link");

    const { headers, status } = await this.requestLink(link);
